...
function init()
  ...
  if not readSpecialStorage() then setSpecialStorage() end
  --sb.logInfo("Init called!")
end
...
function updateTenantRep()
  if isOccupied() and primaryTenant() then
    local tenant = primaryTenant()
    local prevRep = storage.tenantHappiness
    local repBonus = world.callScriptedEntity(tenant, "getRepBonus")

    --sb.logInfo("Prev: " .. sb.print(prevRep) .. " New: " .. sb.print(repBonus))
    
    setRepBonus(repBonus)
  end
end
...
function getRentLevel()
  ...
  local level = getLevelFromTags()
  
  --sb.logInfo("Rent Level Final: " .. level)
  
  if level <= 0 then level = 11 end
...
function chooseTenants(seed, tags)
  ...
  --sb.logInfo("Applicable tenants:")
  for _,tenant in ipairs(matches) do
    --util.debugLog("  " .. tenant.name .. " (priority " .. tenant.priority .. ")")
	--sb.logInfo("  " .. tenant.name .. " (priority " .. tenant.priority .. ")")
  end
  ...
  for _,tenant in ipairs(occupier.tenants) do
    if type(tenant.species) == "table" then
	  ...
	  --sb.logInfo("Tenant Species: " .. tenant.species)
    end
...
function sendNewTenantNotification()
  ...
  if owner then
    ...
    local numDeeds = #deedsNearby
    if numDeeds < 4 then setGoodSpacing(true) else setGoodSpacing(false) end
...
function setSpecialStorage()
  storage.rentOverTimeBonus = rentOverTimeBonus
  storage.tenantHappiness = tenantHappiness
  storage.rentMemory = rentMemory
end

function readSpecialStorage()
  flag = false
  
  if(storage.rentOverTimeBonus ~= nil) then
    rentOverTimeBonus = storage.rentOverTimeBonus
    flag = true
  end
  if storage.tenantHappiness ~= nil then
    tenantHappiness = storage.tenantHappiness
    flag = true
  end
  if storage.rentMemory ~= nil then
    rentMemory = storage.rentMemory
    flag = true
  end
